...
namespace Ornament.Identity.Dao
{
    ...
    public class RoleStore<TRole, TContext, TKey> :
        RoleStore<TRole, TContext, TKey, IdentityRoleClaim<TKey>>,
    ...
    public abstract class RoleStore<TRole, TContext, TKey, TRoleClaim> :
        IQueryableRoleStore<TRole>,
        IRoleClaimStore<TRole>
        ...
        where TRoleClaim : IdentityRoleClaim<TKey>
    {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return
                await
                    RoleClaims.Where(rc => rc.RoleId.Equals(role.Id))
        ...
        /// </param>
        /// <returns>The <see cref="Task" /> that represents the asynchronous operation.</returns>
        public Task AddClaimAsync(TRole role, Claim claim,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ThrowIfDisposed();
            if (role == null)
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var claims =
                await
                    RoleClaims.Where(
                        rc => rc.RoleId.Equals(role.Id) && rc.ClaimValue == claim.Value && rc.ClaimType == claim.Type)
                        .ToListAsync(cancellationToken);
