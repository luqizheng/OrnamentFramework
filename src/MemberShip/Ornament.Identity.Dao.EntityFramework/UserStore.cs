...
namespace Ornament.Identity.Dao
{
        ...
        where TUserToken : IdentityUserToken<TKey>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var entry =
                await
                    UserTokens.SingleOrDefaultAsync(
                        l => l.UserId.Equals(userId) && l.LoginProvider == loginProvider && l.Name == name,
                        cancellationToken);
            if (entry != null)
            {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var entry =
                await
                    UserLogins.SingleOrDefaultAsync(
                        userLogin =>
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (roleEntity != null)
            {
                var userRole =
                    await
                        UserRoles.FirstOrDefaultAsync(r => roleEntity.Id.Equals(r.RoleId) && r.UserId.Equals(user.Id),
                            cancellationToken);
                if (userRole != null)
                {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            /*var query = from userRole in UserRoles
                ...
                select role.Name;*/
            IList<string> result = new List<string>(user.Roles.Select(s => s.Name));
            return result;
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return user.Roles.Any(s => s.NormalizedName.ToLower() == normalizedRoleName.ToLower());
            //return false;
        }
        ...
        private Task<TUserToken> FindToken(TUser user, string loginProvider, string name,
            CancellationToken cancellationToken)
        {
            var userId = user.Id;
            return
