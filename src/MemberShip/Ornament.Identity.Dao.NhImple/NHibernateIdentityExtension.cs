...
// ReSharper restore CheckNamespace
{
    public static class NHibernateIdentityExtension
    {
        public static IdentityBuilder AddNhibernateStores(this IdentityBuilder builder, NhConfigureBuilder nhbuilder)
        {
            ...
            RegistPermissionRelative(nhbuilder, roleIdType, builder.RoleType, builder.UserType, userIdType,
                builder.Services);
            return builder;
        }
        ...
        {
            var permissionType = typeof (Permission<,>).MakeGenericType(roleType, roleIdType);
            nhBuilder.AddType(typeof (PermissionMapping<,,>).MakeGenericType(permissionType, roleType, roleIdType));

            //permission
            var permissionServer1 = typeof (IUserPermissionStore<,,,,>).MakeGenericType(permissionType, userType,
