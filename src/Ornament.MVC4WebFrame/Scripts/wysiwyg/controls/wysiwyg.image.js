    ...
    */
    $.wysiwyg.controls.image = {
        ...
        init: function (Wysiwyg) {
            var self = this, elements, adialog, dialog, formImageHtml, regexp, dialogReplacements, key, translation, footer,
				img = {
				    alt: "",
            ...
            formImageHtml = '<form class="form-horizontal" id="wysiwyg-addImage">' +
				'<div class="control-group"><span class="control-label">{preview}:</span><div class="controls"><img src="" alt="{preview}" style="margin: 2px; padding:5px; max-width: 100%; overflow:hidden; max-height: 100px; border: 1px solid rgb(192, 192, 192);"/></div></div>' +
				'<div class="control-group"><label for="name" class="control-label">{url}:</label><div class="controls"><input type="text" name="src" value=""/>';
            ...
            formImageHtml += '</div></div>' +
		        '<div class="control-group"><label for="name" class="control-label">{title}:</label><div class="controls"><input type="text" name="imgtitle" /></div></div>' +
		        '<div class="control-group"><label for="name" class="control-label">{description}:</label><div class="controls"><input type="text" name="description" /></div></div>' +
		        '<div class="control-group"><label for="name" class="control-label">{width} x {height}:</label><div class="controls controls-row"><input type="text" name="width" class="span1"/><input type="text" name="height" class="span1"/></div></div>' +
		        '<div class="control-group"><label for="name" class="control-label">{original}:</label><div class="controls controls-row"><input type="text" name="naturalWidth"  class="span1" disabled="disabled"/>' +
		        '<input type="text" name="naturalHeight"  class="span1" disabled="disabled"/></div></div>' +
		        '<div class="control-group"><label for="name" class="control-label">{float}:</label><div class="controls"><select name="float">' +
		        '<option >{floatNone}</option>' +
		        '<option value="left">{floatLeft}</option>' +
		        '<option value="right">{floatRight}</option></select></div></div></div>';

            footer = '<input type="submit" class="btn btn-primary" value="{submit}" id="wysiwyg-addImage-ok"/> ' +
            ...
            for (key in dialogReplacements) {
                ...
                footer = footer.replace(regexp, dialogReplacements[key]);
            }
            ...
            $(adialog).bind("afterOpen", function (e, dialog) {
                dialog.find("#wysiwyg-addImage-ok").click(function (e) {
                    e.preventDefault();
