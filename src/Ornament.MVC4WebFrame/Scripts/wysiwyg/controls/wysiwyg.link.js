	...
	*/
	$.wysiwyg.controls.link = {
		init: function (Wysiwyg) {
			var self = this, $element, dialog, url, a, selection,
			...
			if ($.fn.dialog) {
				$element = $(formLinkHtml);
				$element.find("input[name=linkhref]").val(a.href);
				$element.find("input[name=linktitle]").val(a.title);
				$element.find("input[name=linktarget]").val(a.target);

				if ($.browser.msie) {
					try {
						dialog = $element.appendTo(Wysiwyg.editorDoc.body);
					} catch (err) {
