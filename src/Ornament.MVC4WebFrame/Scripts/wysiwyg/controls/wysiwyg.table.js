...
(function ($) {
    ...
    */
    $.wysiwyg.controls.table = function (Wysiwyg) {
        var adialog, dialog, colCount, rowCount, formTableHtml, dialogReplacements, key, translation, regexp, footerTableHtml;
        ...
        formTableHtml = '<div class="form-horizontal" id="wysiwyg-tableInsert">' +
            '<div class="controls-group"><label class="control-label">{cols}</label><div class="controls"><input type="text" name="colCount" value="3" /></div></div>' +
            '<div class="controls-group"><label class="control-label">{rows}</label><div class="controls"><input type="text" name="rowCount" value="3" /></div></div>' +
            '</div>';
        footerTableHtml = '<a class="btn btn-primary button" value="" id="wysiwyg-tableInsert-ok">{submit}</a> ' +
			'<input type="reset" value="{reset}" class="btn" id="wysiwyg-tableInsert-reset"/>';
        ...
        for (key in dialogReplacements) {
            ...
            footerTableHtml = footerTableHtml.replace(regexp, dialogReplacements[key]);
        }
        ...
        adialog = new $.wysiwyg.dialog(Wysiwyg, {
            ...
            footer: footerTableHtml,
            "open": function (e, dialog) {

                dialog.find("#wysiwyg-tableInsert-ok").click(function (e) {
                    e.preventDefault();
