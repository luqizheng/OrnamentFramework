/*==================================================
 ...
 avalon.js 1.391 build in 2015.2.7 
...
(function(global, factory) {
     ...
     *                    全局变量及方法                                  *
     **********************************************************************/
    var expose = new Date - 0; //http://stackoverflow.com/questions/7290086/javascript-use-strict-and-nicks-find-global-function
...
//生成UUID http://stackoverflow.com/questions/105034/how-to-create-a-guid-uuid-in-javascript
    ...
    avalon.isWindow = function(obj) {
        if (!obj)
            return false; // 利用IE678 window == document为true,document == window竟然为false的神奇特性
...
/*********************************************************************
    ...
    avalon.contains = fixContains; //safari5+是把contains方法放在Element.prototype上而不是Node.prototype
    if (!root.contains) {
...
//IE9-11,firefox不支持SVG元素的innerHTML,outerHTML属性
                    ...
                    set: function(html) {
                        var tagName = this.tagName.toLowerCase(),
                            par = this.parentNode,
                            frag = avalon.parseHTML(html); // 操作的svg，直接插入
                        if (tagName === "svg") {
    ...
    var eventHooks = avalon.eventHooks; //针对firefox, chrome修正mouseenter, mouseleave
...
//ms-with,ms-each, ms-repeat绑定生成的代理对象储存池
    ...
    var canHideOwn = true; //如果浏览器不支持ecma262v5的Object.defineProperties或者存在BUG，比如IE8
...
//===================修复浏览器对Object.defineProperties的支持=================
        ...
        if (IEVersion) {
            ...
            defineProperties = function(name, accessors, properties) {
                ...
                buffer.push(
                    ...
                    "\tEnd Function"); //添加普通属性,因为VBScript对象不能像JS那样随意增删属性，必须在这里预先定义好
                ...
                buffer.push("\tPublic [" + 'hasOwnProperty' + "]"); //添加访问器属性 
        ...
        if (diff) {
            ...
            while (obj = $$subscribers[--i]) {
                ...
                if (needTest[data.type] && isRemove(data.element)) { //如果它没有在DOM树
                    ...
                    avalon.Array.remove(obj.list, data); //log("debug: remove " + data.type)
                    disposeData(data);
                    obj.data = obj.list = null;
                }
            }
        }
        oldInfo = newInfo; // avalon.log("已经移除的个数 " + k)
        beginTime = new Date();
    }
    ...
    function scanTag(elem, vmodels, node) {
        ...
        var a = elem.getAttribute("ms-skip"); //#360 在旧式IE中 Object标签在引入Flash等资源时,可能出现没有getAttributeNode,innerHTML的情形
    ...
    var rnoscanNodeBinding = /^each|with|html|include$/; //IE67下，在循环绑定中，一个节点如果是通过cloneNode得到，自定义属性的specified为false，无法进入里面的分支，
    ...
    if (!"1"[0]) {
        ...
        var rattrs = /\s+(ms-[^=\s]+)(?:=("[^"]*"|'[^']*'|[^\s>]+))?/g,
            rquote = /^['"]/,
            rtag = /<\w+\b(?:(["'])[^"]*?(\1)|[^>])*>/i,
            ramp = /&amp;/g; //IE6-8解析HTML5新标签，会将它分解两个元素节点与一个文本节点
        ...
        var getAttributes = function(elem) {
            var html = elem.outerHTML; //处理IE6-8解析HTML5新标签的情况，及<br>等半闭合标签outerHTML为空的情况
    ...
    "add,remove".replace(rword, function(method) {
        avalon.fn[method + "Class"] = function(cls) {
            var el = this[0]; //https://developer.mozilla.org/zh-CN/docs/Mozilla/Firefox/Releases/26
...
//=============================css相关=======================
        ...
        cssHooks["@:get"] = function(node, name) {
            ...
            if ((rnumnonpx.test(ret) && !rposition.test(ret))) {
                ...
                var style = node.style,
                    left = style.left,
                    rsLeft = node.runtimeStyle.left; //②由于③处的style.left = xxx会影响到currentStyle.left，
                //因此把它currentStyle.left放到runtimeStyle.left，
                //runtimeStyle.left拥有最高优先级，不会style.left影响
                node.runtimeStyle.left = currentStyle.left; //③将精确值赋给到style.left，然后通过IE的另一个私有属性 style.pixelLeft
                //得到单位为px的结果；fontSize的分支见http://bugs.jquery.com/ticket/760
                style.left = name === 'fontSize' ? '1em' : (ret || 0);
                ret = style.pixelLeft + "px"; //④还原 style.left，runtimeStyle.left
                style.left = left;
                node.runtimeStyle.left = rsLeft;
            }
            if (ret === "medium") {
                name = name.replace("Width", "Style"); //border width 默认值为medium，即使其为0"
    ...
    avalon.fn.offset = function() { //取得距离页面左右角的坐标
        ...
        var clientTop = root.clientTop || body.clientTop,
            ...
            scrollLeft = Math.max(win.pageXOffset || 0, root.scrollLeft, body.scrollLeft); // 把滚动距离加到left,top中去。
    ...
    var valHooks = {
        ...
        "select:get": function(node, value) {
            ...
            for (; i < max; i++) {
                option = options[i]; //旧式IE在reset后不会改变selected，需要改用i === index判定
...
//缓存求值函数，以便多次利用
    var cacheExprs = createCache(128); //取得求值函数及其传参
    var rduplex = /\w\[.*\]|\w\.\w/;
    ...
    function parseExpr(code, scopes, data) {
        ...
        var vars = getVariables(code).concat(),
            ...
            names = [],
            args = [],
            prefix = ""; //args 是一个对象数组， names 是将要生成的求值函数的参数
    ...
    function parseExprProxy(code, scopes, data, tokens, noregister) {
        ...
        if (data.evaluator && !noregister) {
            data.handler = bindingExecutors[data.handlerName || data.type]; //方便调试
...
//属性名映射
            ...
            if (data.param === "src") {
                ...
                if (el) {
                    if (el.tagName === "NOSCRIPT" && !(el.innerHTML || el.fixIE78)) { //IE7-8 innerText,innerHTML都无法取得其内容，IE6能取得其innerHTML
                        ...
                        xhr.send(null); //http://bbs.csdn.net/topics/390349046?page=1#post-393492653
    ...
    var widgetList = []; //不存在 bindingExecutors.widget
    //双工绑定
...
//当model变化时,它就会改变value的值
        ...
        if (data.isChecked || element.type === "radio") {
            ...
            events.replace(rword, function(name) {
                ...
                case "input":
                    if (!IEVersion) { // W3C
                        bound("input", updateVModel); //非IE浏览器才用这个
...
//取得上午下午
    ...
    new function() {
        ...
        function fireRequest(req) {
            var name = req.name;
            var res = req.res; //1. 如果该模块已经发出请求，直接返回
        ...
        innerRequire.config = kernel; //核心API之四 define.amd 标识其符合AMD规范
        innerRequire.define.amd = modules; //==========================对用户配置项进行再加工==========================
        ...
        function toUrl(id) {
            ...
            url = urlNoQuery + this.query; //6. 处理urlArgs
