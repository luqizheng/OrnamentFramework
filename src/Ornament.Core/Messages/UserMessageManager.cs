...
namespace Ornament.Messages
{
    public static class UserMessageManager
    {
        ...
        {
            var result = new Message(manager.Admin, manager.ChangePasswordMessageType)
                {
            ...
            manager.InfoDaoFactory.MessageDao.SaveOrUpdate(result);
        }
        ...
        {
            var result = new Message(manager.Admin, manager.CreateUserMessageType)
                {
                    State = MessageState.Published,
                    Priority = Priority.Medium
                };
            ...
            var content = new Content
                {
                    Value =
                        String.Format("create new user {0},loginid is {0} by {1}(loginid={2})", newUser.Name,
                                      newUser.LoginId, @operator.Name, @operator.LoginId)
                };
        ...
        public static void RegistryNewUser(this OrnamentMessageManager manager, User newUser)
        {
            ...
            var result = new Message(manager.Admin, manager.RegistryUserMessageType);
            var content = new Content
                {
                    Value =
                        String.Format("A user which loginid is {0}, registed at {1}", newUser.LoginId,
                                      newUser.CreateTime.ToString("yyyyy-MM-dd HH-mm-ss"))
                };
            result.Contents.Add("", content);
            result.AddReaders(manager.MemberShipFactory.CreateUserGroupDao().GetByName("administrators"));
            manager.InfoDaoFactory.MessageDao.SaveOrUpdate(result);
