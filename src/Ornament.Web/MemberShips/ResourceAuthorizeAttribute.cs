...
namespace Ornament.Web.MemberShips
{
    public class ResourceAuthorizeAttribute : FilterAttribute, IAuthorizationFilter
    {
        ...
        public ResourceAuthorizeAttribute(object @operator, Type resourceType, string resourceId)
        {
            if (@operator == null)
        ...
        protected bool AuthorizeCore(HttpContextBase context)
        {
            ...
            {
                var res =
                    OrnamentContext.Current.GetDaoFactory<IMemberShipFactory>().CreateResourceDao().Load(
