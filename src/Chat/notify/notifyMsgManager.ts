...
import NotifyMsg=require("../entities/notifyMsg");
import orgSetting = require("../Config/orgSetting");
import dbOp = require("../dao");
var db = dbOp.getProvider("sendBox");

function toContent(strTemplate:string, variabled:Array, user:user) {
    var reg = /\[{1}[^[].+?\]{1}/g
...
export  function count (loginId:string, bRead:boolean, callback:Function) {
    db.do(function (collection) {
        var codi = {Owner: loginId};
        if (bRead !== undefined) {
            codi.IsRead = bRead;
...
exports function Init(socket, userManager) {

    socket.on('send notify', function (msg:NotifyMsg.notifyMsg) {
